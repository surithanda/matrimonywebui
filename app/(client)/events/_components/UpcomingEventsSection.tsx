...
const UpcomingEventsSection = () => {
  return (
    <section className="pb-16 px-4 sm:pb-20 sm:px-6 md:pb-24 md:px-8 lg:pb-28 lg:px-12 xl:pb-[120px] xl:px-[120px]">
      {/* Section Heading */}
      <h2 className="mb-8 sm:mb-10 md:mb-12 lg:mb-16 xl:mb-[50px] BRCobane56600 text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl text-center leading-tight">
        Upcoming Events
      </h2>

      {/* Events Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 md:gap-6 lg:gap-8 max-w-7xl mx-auto">
        {events.map((event, index) => (
          <div
            key={index}
            className="relative group hover:transform hover:scale-105 transition-all duration-300 ease-in-out"
          >
            {/* Event Image */}
            <div className="relative overflow-hidden rounded-lg sm:rounded-xl mb-4 sm:mb-5 md:mb-6">
              <Image
                src={event.imageUrl}
                alt={event.alt}
              ...
              {/* Tickets Left Badge */}
              <div className="p-1 sm:p-[2px_8px] md:p-[2px_12px] rounded-[8px] sm:rounded-[10px] flex gap-1 border border-gray-200 w-fit items-center absolute top-2 left-2 sm:top-2.5 sm:left-2.5 bg-white shadow-sm">
                <svg
            ...
            <div className="px-1 sm:px-2">
              {/* Title and Price */}
              <div className="flex w-full justify-between items-start gap-3 mb-3 sm:mb-4 md:mb-[15px]">
                <h3 className="BRCobane24400 text-lg sm:text-xl md:text-2xl leading-tight flex-1 min-w-0">
                  {event.title}
                </h3>
                <div className="p-1.5 sm:p-[2px_8px] md:p-[2px_12px] rounded-[8px] sm:rounded-[10px] bg-[#FEAC00] text-sm sm:text-base font-semibold text-black flex-shrink-0">
                  {event.price}
                </div>
              </div>

              {/* Location */}
              <div className="BRCobane16400 flex items-center gap-2 sm:gap-3 md:gap-5 mb-2 sm:mb-3 md:mb-[15px]">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
              ...
              {/* Date */}
              <div className="BRCobane16400 flex items-center gap-2 sm:gap-3 md:gap-5">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
